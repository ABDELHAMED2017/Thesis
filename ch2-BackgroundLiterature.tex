...
This transceiver in a multicarrier system can be modelled as a block diagram, shown in Figure~\ref{fig:multicarrier-block}.
...
%FBMC systems can be categorised into two distinct classes \cite{Farhang-Boroujeny2011}.
%The first class are designed to transmit complex-valued (i.e. quadrature amplitude-modulated (QAM)) data symbols.
%The second class are for transmission of real-valued (i.e. pulse amplitude-modulated (PAM)) data symbols.
%FBMC systems were originally introduced in the 1960s even before the development of OFDM systems.
%A number of recent research papers have shown the potential of FBMC for efficient broadband spectrum usage in multiple access applications, including cognitive radios.
...
OFDM is a multicarrier modulation scheme used in both wireline and wireless communication in which a high-rate data stream is split into multiple parallel low rate streams that are modulated by multiple subcarriers.
...
In order to avoid ISI, a guard interval (or cyclic prefix), having a length of $L_{CP}$, must be added before each OFDM symbol as demonstrated in Figure~\ref{fig:CIR-CP}.
...
Figure~\ref{fig:Timingoffsetconstellation} illustrates the effect of timing offset on a single 256 subcarrier OFDM symbol utilizing QPSK subcarriers (based on the IEEE~802.16 standard).
...
%The effect of ISI caused by later timing offsets on the OFDM received symbol is illustrated in Figure~\ref{fig:Timingoffsetconstellation}(a) and Figure~\ref{fig:Timingoffsetconstellation}(c).
%The effect of subcarrier rotation caused by earlier timing offsets on the OFDM received symbol is illustrated in Figure~\ref{fig:Timingoffsetconstellation}(b) and Figure~\ref{fig:Timingoffsetconstellation}(d).
...
One drawback of OFDM is spectral leakage due to the summation of sinusoidal subcarriers windowed by a rectangular function.
...
The four strict 802.11p SEMs are defined to reduce the effect of ICI between the channels. %, shown in Figure~\ref{fig:4SEM},
...
Last but not least, reducing power dissipation has become a crucial issue in wireless communication systems, especially for portable devices.
