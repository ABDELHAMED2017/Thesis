...
The \emph{Shift_Add} block, multiplexers and value of $Pr[n]$ are different depending upon the quantised coefficient set being used, and these perform shifts and additions on received samples according to the degree of quantisation that is applied.
To optimise resources in the case of small numbers of bit quantisation, one common \emph{Shift_Add} block is used for all 64 coefficients instead of 64 separate \emph{Shift_Add} blocks.
This common \emph{Shift_Add} block calculates all possible values for 64 coefficients.
The multiplexers are used to select the corresponding \emph{Shift_Add} outputs to accumulate in order to generate the correlator output.
