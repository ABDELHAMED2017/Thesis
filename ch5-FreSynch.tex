...
However the CFO estimation of that method was only estimate the fractional CFO rather that large integer CFO that can occur as a result of the Doppler Effect and/or due to local oscillator instabilities.
...
Let us assume that the signal is transmitted over a frequency selective channel that has channel impulse response (CIR) $h$ with length, $L$ ($L<N_{CP}$), and which is corrupted by AWGN. 
...
This results in the possible IFO values being a multiple of 4. Readers should refer to \cite{Kim2008} to understand in detail the limitation of fractional CFO and the possible IFO values.
