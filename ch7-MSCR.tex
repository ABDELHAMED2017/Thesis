...
The reconfiguration is required as fast as that the input data buffered during reconfiguration does not caused the buffer overflow that leads to the packet of data dropping. Even there are higher-level protocols that manage the retransmission of lost or dropped packet of data, but performing these mechanisms are wasteful and significant increase the cost in terms of computation and power consumption.
...
In this Section, we present the works to check the correctness and performance of system.
...
\begin{figure}
	...
	\label{fig:Ver}
\end{figure}
Fig.~\ref{fig:Ver} illustrates the verification framework. The comprehensive verifications are performed based on the comparison of the signal vectors of implemented system using Verilog HDL and the corresponding signal vectors of a system golden model simulated using Matlab.
In addition, Matlab is used to generate random data as input vectors. The common input vectors are used to perform OFDM modulation by both of the system simulation and the system implementation. Test bench scripts are wrote to send the input vectors from database to the implemented system. Moreover, test bench scripts are also used to monitor the signals generated by the system and store it to a database as output vectors. Then the output vectors are read to the Matlab for comparison to the corresponding vectors generated by simulation.
...
Fig.~\ref{fig:VerSys} shows the block diagram od the baseband system under verification. The necessary components are connected to a transmitter and a receiver. The output of the transmitter is buffered and connected to the receiver. There are many intermediate check points at the output of sub-modules that are observed from implementation and simulation for verification.

The results of verification shown in Fig.~\ref{fig:TX_Ver}, Fig.~\ref{fig:RX_Ver}, and Fig.~\ref{fig:Sys_Ver} verify the system performing baseband modulation of 802.11 signal for an instance. It's similar to the verification for other standards. \emph{MM\_sim},
\emph{MM\_rtl} denote the output of \emph{MM} submodule in simulation and rtl implementation, respectively. The waveforms of simulation and rtl implementation are almost identical that proves the correctness of the implemented system.	
...
Fig.~\ref{fig:TX_Ver} illustrates the verification results of the transmitter. At the left hand side figure, we can see that the data symbols and the pilots are inserted in the OFDM symbols. The right hand side figure  shows an OFDM signal at the output of transmitter including preambles and OFDM symbols.
...
Fig.~\ref{fig:RX_Ver} presents the verification results of the receiver. At the top-left  figure, synchronised signal is illustrated, while the top-right figure shows the signal removing symbol cyclic prefix. The bottom-left plots the demodulated data symbols after performing FFT. The output of FFT contains a long preamble that is employed by the subsequent module that perfroms IFO estmation and Channel equalisation. The output of this module is shown in bottom-right figure.
...
Fig.~\ref{fig:Sys_Ver} presents the observation of the bit streams at system input and system output. At the left hand side figure, we can see that comparison of output bit stream between rtl implementation and simulation. Because the QPSK is applied for data modulation, A data symbol is modulated for 2 data bits. Therefore, the value is presented from 0 to 3 corresponding to 00, 01, 10, 11 in binary numbers. The right hand side figure shows the comparison between input and output bit streams of rtl implementation. The data bit stream is modulated at OFDM based band and then demodulated. The input and output streams are identical each other.

Based on the verification framework, the performance of system implementation is verified with regard to the golden model. The results prove the correctness of the implementation.
